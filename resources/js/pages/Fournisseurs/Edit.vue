...
const form = useForm({
    ...
    num_tel_fourn: props.fournisseur.num_tel_fourn,
});
</script>

<template>
    <Head title="Edit Fournisseur" />
    <AppLayout>
        <div class="container mx-auto p-4 sm:p-6 lg:p-8">
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-6">Edit Fournisseur</h1>

            <form @submit.prevent="form.put(`/fournisseurs/${props.fournisseur.nom_fourn}`)"
                  class="max-w-xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-200">
                <div class="mb-5">
                    <label for="nom_fourn" class="block text-sm font-medium text-gray-700 mb-2">Nom:</label>
                    <input
                ...
                <div class="mb-8">
                    <label for="num_tel_fourn" class="block text-sm font-medium text-gray-700 mb-2">Téléphone:</label>
                    <input
                        v-model="form.num_tel_fourn"
                        id="num_tel_fourn"
                        type="text"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm"
                        :class="{ 'border-red-500': form.errors.num_tel_fourn }"
                    >
                    <p v-if="form.errors.num_tel_fourn" class="mt-2 text-sm text-red-600">{{ form.errors.num_tel_fourn
                        }}</p>
                </div>
