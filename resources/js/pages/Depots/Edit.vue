...
const form = useForm({
    adresse_depot: props.depot.adresse_depot
});
</script>
...
<template>
    ...
    <AppLayout>
        <div class="container mx-auto p-4 sm:p-6 lg:p-8">
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-6">Modifier le Dépôt</h1>

            <form @submit.prevent="form.put(`/depots/${depot.id_depot}`)" class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-200">
                <div class="mb-5">
                    <label for="adresse_depot" class="block text-sm font-medium text-gray-700 mb-2">Adresse du Dépôt:</label>
                    <input
                        ...
                        :class="{ 'border-red-500': form.errors.adresse_depot }"
                    >
                    <p v-if="form.errors.adresse_depot" class="mt-2 text-sm text-red-600">{{ form.errors.adresse_depot }}</p>
                </div>
