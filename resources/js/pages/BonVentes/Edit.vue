...
// Initialize the form with data from the existing Bon de Vente
const form = useForm({
    // Split the date string to get only the date part for the input field
    date_bv: (props.bonVente.date_bv || '').split('T')[0],
    nom_client: props.bonVente.nom_client,
    // Use the existing products array
    products: props.bonVente.products,
    montant_verse: props.bonVente.montant_verse,
});

// Computed property to calculate the total amount of the sale
const totalAmount = computed(() => {
    return form.products.reduce((total, product) => {
        // Ensure qte_vente and prix_vente are numbers before multiplying
        const quantity = parseFloat(product.qte_vente) || 0;
        const price = parseFloat(product.prix_vente) || 0;
        return total + (quantity * price);
    }, 0);
});

// Computed property to calculate the remaining amount to be paid
const resteAPayer = computed(() => {
    // Ensure totalAmount and montant_verse are numbers before calculating
    return (totalAmount.value - (parseFloat(form.montant_verse) || 0)).toFixed(2);
});

// Function to add a new product line to the form
const addProduct = () => {
    form.products.push({ id_prod: '', qte_vente: 1, prix_vente: 0 });
};

// Function to remove a product line from the form
...
const submitForm = () => {
    // Add the 'etat_bv' field based on the payment status before submitting
    form.transform((data) => ({
        ...data,
        etat_bv: parseFloat(data.montant_verse) >= totalAmount.value ? 'paye' : 'verse'
    })).put(`/bon-ventes/${props.bonVente.n_bv}`);
};
</script>

<template>
    <Head title="Modifier Bon de Vente" />
    <AppLayout>
        <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-6">Modifier Bon de Vente #{{ bonVente.n_bv }}</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            :class="{ 'border-red-500': form.errors.date_bv }"
                        >
                        <p v-if="form.errors.date_bv" class="mt-2 text-sm text-red-600">{{ form.errors.date_bv }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm"
                            :class="{ 'border-red-500': form.errors.nom_client }"
                        >
                        ...
                        <p v-if="form.errors.nom_client" class="mt-2 text-sm text-red-600">{{ form.errors.nom_client }}</p>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 pt-6 mt-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-3">
                            ...
                            <p class="mt-1 text-xl font-extrabold text-gray-900">{{ resteAPayer }} DA</p>
                        </div>
                    </div>
                </div>
